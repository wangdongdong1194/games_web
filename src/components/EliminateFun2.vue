...
<script lang="ts" setup>
    ...
    async function start() {
        ...
        for (let i = 8; i >= 0; i--) {
            ...
            if (clearRows.columnArray.length) {
                resultArray.push(clearRows);
            }
        }
        for (let i = 8; i >= 0; i--) {
            const clearColumns = canClearColumn(i);
            if (clearColumns.columnArray.length) {
                resultArray.push(clearColumns);
            }
        }
        for (const res of resultArray) {
            if (res.cellIndexArray.length) {
                // 标记消除格子
                res.cellIndexArray.forEach((line) => line.forEach((index) => eliminates.value[index].clear = true));
            }
        }
        await sleep();
        for (const res of resultArray) {
            // 等待动画结束后，执行下落
            const fulldownResult = fulldown(res.columnArray);
    ...
    async function sleep(sec: number = 0.3) {
